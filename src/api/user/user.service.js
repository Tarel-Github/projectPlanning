...
class UserService {
    ...
    signUp = async (nickname, email, password) =>{
        ...
        password = await bcrypt.hash(password,Number(process.env.BCRYPT_SALT_ROUNDS));
    ...
    login = async (email, password) => {
        ...
        if(!loginUser || !(await bcrypt.compare(password, loginUser.get().password)))//입력받은 password와 리포서 가져온 비번을 비교
